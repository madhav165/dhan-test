create table candles (
  security_id      text        not null,
  exchange_segment text        not null,
  interval         text        not null,
  timestamp        timestamptz not null,
  open             numeric     not null,
  high             numeric     not null,
  low              numeric     not null,
  close            numeric     not null,
  volume           bigint      not null,
  primary key (security_id, exchange_segment, interval, timestamp)
);

create index idx_candles_lookup on candles (security_id, exchange_segment, interval, timestamp desc);

create table strategies (
  id          uuid primary key default gen_random_uuid(),
  user_id     uuid references users(id) on delete cascade,
  name        text        not null,
  interval    text        not null,
  from_date   date        not null,
  to_date     date        not null,
  status      text        not null default 'draft',  -- draft / backtesting / active
  mode        text        not null default 'alert',  -- alert / trade
  source_key  text,
  wasm_key    text,
  created_at  timestamptz default now()
);

create index idx_strategies_user on strategies (user_id);

create table strategy_instruments (
  strategy_id      uuid references strategies(id) on delete cascade,
  security_id      text not null,
  exchange_segment text not null,
  primary key (strategy_id, security_id, exchange_segment)
);

create table backtest_runs (
  id           uuid primary key default gen_random_uuid(),
  strategy_id  uuid references strategies(id) on delete cascade,
  run_at       timestamptz default now(),
  result_key   text,
  num_trades   int,
  total_pnl    numeric,
  win_rate     numeric,
  max_drawdown numeric
);

create index idx_backtest_runs_strategy on backtest_runs (strategy_id);

create table live_signals (
  id           uuid primary key default gen_random_uuid(),
  strategy_id  uuid references strategies(id) on delete cascade,
  security_id  text        not null,
  triggered_at timestamptz not null,
  signal       text        not null,
  price        numeric     not null
);

create index idx_live_signals_strategy on live_signals (strategy_id, triggered_at desc);
